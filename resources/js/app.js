...
moment.tz.setDefault(Profiler.timezone);

window.Profiler.basePath = '/' + window.Profiler.path;

let routerBasePath = window.Profiler.basePath + '/';

if (window.Profiler.path === '' || window.Profiler.path === '/') {
    routerBasePath = '/';
    window.Profiler.basePath = '';
}
...
new Vue({
    ...
    methods: {
        ...
        toggleRecording() {
            axios.post(Profiler.basePath + '/profiler-api/toggle-recording');

            window.Profiler.recording = !Profiler.recording;
